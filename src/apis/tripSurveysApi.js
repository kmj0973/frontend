...
export async function postSurveyData(surveyData, inviteCode) {
  ...
  try {
    ...
    console.error(
      SURVEY_API_ERROR_MESSAGE.SUBMIT_ERROR_REASON,
...
export const resetSurveyData = async (tripGroupId) => {
  const response = await instance.post(`/v1/trip-groups/${tripGroupId}/reset`);
  return response.data;
};
